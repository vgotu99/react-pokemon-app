    ...
    <div className="relative z-50">
      ...
      {checkEqualName(searchTerm).length > 0 && (
        <div className={`w-full flex bottom-0 h-0 flex-col absolute justify-center items-center translate-y-2`}>
          ...
          <ul className="pl-2 w-40 max-h-[134px] py-1 bg-gray-700 rounded-lg absolute top-0 overflow-auto scrollbar-none">
            {checkEqualName(searchTerm).map((e, i) => (
              <li key={`button-${i}`}>
                <butto className='text-base w-full hover:bg-gray-600 p-[2px] text-gray-100'>{e.name}</butto>
              </li>
            ))}
          </ul>
        </div>
